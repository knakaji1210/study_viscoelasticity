...
def calc_relaxFunc(E, k, T, t):
    relaxFunc = (1/k + (1-1/k)*np.exp(-t/T))*E
    return relaxFunc
...
if __name__=='__main__':
    ...
    fit_result = 'slope of relaxation modulus: {0:.2f}'.format(paramComp[0])
    ...
    ax1.set_ylabel('log(Relaxation modulus /Pa)')
    ax1.set_xlim(Times[0],Times[1])
    ax1.set_ylim(np.min(log_relaxFunc)-1, np.max(log_relaxFunc)+1)
    ax1.grid()
    ax1.scatter(log_time, log_relaxFunc, c='r', label='Relaxation modulus')
    ax1.plot(log_time, fit_relaxFunc, c='b', ls=':', label='fitted Relaxation modulus')
    ax1.legend(loc='upper right')

    fig.text(0.6, 0.7, fit_result)

    fig.savefig('./png/sls_model_relax.png')
