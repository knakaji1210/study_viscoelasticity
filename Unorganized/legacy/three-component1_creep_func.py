...
def calc_creepFunc(E, k, T, t):
    creepFunc = -1 + k*(1 - (1 - 1/k)*np.exp(-t/T))
    #creepFunc = k*(1 - (1 - 1/k)*np.exp(-t/T))   #Johnsonの定義
    return creepFunc
...
if __name__=='__main__':
    ...
    fit_result = 'slope of creep function: {0:.2f}'.format(paramComp[0])
    ...
    ax1.set_ylabel('log(Creep function)')
    ax1.set_xlim(Times[0],Times[1])
    ax1.set_ylim(np.min(log_creepFunc)-1, np.max(log_creepFunc)+1)
    ax1.grid()
    ax1.scatter(log_time, log_creepFunc, c='r', label='Creep funcion')
    ax1.plot(log_time, fit_creepFunc, c='b', ls=':', label='fitted Creep funcion')
    ax1.legend(loc='upper left')

    fig.text(0.6, 0.3, fit_result)

    fig.savefig('./png/sls_model_creep_func.png')
