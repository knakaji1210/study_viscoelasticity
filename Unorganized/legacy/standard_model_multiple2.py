...
if __name__=='__main__':
    ...
    for i in range(params[0]):
        # 全ての要素にk0を追加するバージョン
        strMod = [s1 + s2 for (s1, s2) in zip(strMod, dynamicMod(params[1], params[2][i], params[3][i], freq)[0])]
        losMod = [l1 + l2 for (l1, l2) in zip(losMod, dynamicMod(params[1], params[2][i], params[3][i], freq)[1])]
