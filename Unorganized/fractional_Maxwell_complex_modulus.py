...
def freqAxis(relaxTime):
    ...
    freq = np.logspace(int(np.log10(centerFreq))-5, int(np.log10(centerFreq))+5, 51)
...
def fitFreqs():
    try:
        minFreq1 = float(input('Enter minimum frequency for fitting (storage) (default = -5): '))
    except ValueError:
        minFreq1 = -5
    try:
        maxFreq1 = float(input('Enter maximum frequency for fitting (storage) (default = -1): '))
         ...
         maxFreq1 = -1
    try:
        minFreq2 = float(input('Enter minimum frequency for fitting (loss) (default = -5): '))
    except ValueError:
        minFreq2 = -5
    try:
...
if __name__=='__main__':
    ...
    strMod = complexMod(insMod, relaxTime, freq, nu)[0]
    losMod = complexMod(insMod, relaxTime, freq, nu)[1]
    ...
    if select == 0:
        ...
        savefile = './png/fractional_Maxwell_complex_modulus_linear.png'
    ...
    if select == 1:
        ...
        a = 1
        legend_loc='upper left'
        savefile = './png/fractional_Maxwell_complex_modulus_log.png'
        try:
    ...
    if select == 2:
        ...
        a = 0
        legend_loc='upper left'
        savefile = './png/fractional_Maxwell_loss_tangent.png'
    ...
    if fitting == 0:
        ...
        ylim = [np.min(np.minimum(y1, y2))-0.5, np.max(np.maximum(y1, y2))+0.5]
        fig.text(0.7, 0.65, fit_result1)
        fig.text(0.7, 0.60, fit_result2)
