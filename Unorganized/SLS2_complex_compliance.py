# complex compliance of SLS model 2
...
def freqAxis(relaxTime):
    ...
    freq = np.logspace(int(np.log10(centerFreq))-3, int(np.log10(centerFreq))+3, 37)
...
def fitFreqs():
    try:
        minFreq1 = float(input('Enter minimum frequency for fitting (storage) (default = -0.8): '))
    except ValueError:
        minFreq1 = -0.8
    try:
        maxFreq1 = float(input('Enter maximum frequency for fitting (storage) (default = 0): '))
    except ValueError:
         maxFreq1 = 0
    try:
        ...
        minFreq2 = -3
    try:
        maxFreq2 = float(input('Enter maximum frequency for fitting (loss) (default = -1.5): '))
    except ValueError:
         maxFreq2 = -1.5
...
if __name__=='__main__':
    ...
    if select == 0:
        ...
        savefile = './png/SLS2_complex_compliance_linear.png'
    ...
    if select == 1:
        ...
        a = 1
        legend_loc='upper right'
        savefile = './png/SLS2_complex_compliance_log.png'
        try:
    ...
    if select == 2:
        ...
        a = 0
        legend_loc='upper right'
        savefile = './png/SLS2_loss_tangent.png'
    ...
    if fitting == 0:
        ...
        ylim = [np.min(y1)-0.5, np.max(y1)+0.5]
        fig.text(0.3, 0.30, fit_result1)
        fig.text(0.3, 0.25, fit_result2)
