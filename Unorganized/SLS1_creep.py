# creep compliance of SLS model 1
...
def calc_creepComp(E, k, T, t):
    creepComp = (1 - (1 - 1/k)*np.exp(-t/T))/E
    # E must be infMod
    return creepComp
...
def timeAxis(retardTime):
    ...
    Time = np.logspace(log_relaxT-7, log_relaxT+2, 46)
...
def fitTimes():
    try:
        minTime = float(input('Enter minimum time for fitting in log scale (default = -0.9): '))
    except ValueError:
        minTime = -0.9
    try:
