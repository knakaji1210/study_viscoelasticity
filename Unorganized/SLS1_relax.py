# relaxation modulus of SLS model 1
...
def calc_relaxMod(E, k, T, t):
    relaxMod = E*(1/k + (1 - 1/k)*np.exp(-k*t/T))
    # E must be insMod
    return relaxMod
...
def calc_relaxFunc(T, t):
    relaxFunc = (1 - 1/k)*(1 - np.exp(-k*t/T))
    return relaxFunc
...
def timeAxis(relaxTime):
    ...
    Time = np.logspace(log_relaxT-3, log_relaxT+2, 36)
...
def fitTimes():
    try:
        minTime = float(input('Enter minimum time for fitting in log scale (default = -2): '))
    except ValueError:
        minTime = -2
    try:
        maxTime = float(input('Enter maximum time for fitting in log scale (default = -1): '))
    except ValueError:
         maxTime = -1
